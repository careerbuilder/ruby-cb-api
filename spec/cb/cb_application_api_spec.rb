  ...
  describe Cb::ApplicationApi do
    ...
    context 'application submission methods' do
      ...
      { # the #submit_app and #submit_registered_app methods work exactly the same - metaprogram it!
        '#submit_app'            => { :uri => Cb.configuration.uri_application_submit,     :method => :submit_app},
        '#submit_registered_app' => { :uri => Cb.configuration.uri_application_registered, :method => :submit_registered_app}
        ...
        context method_under_test do
          it 'raises exception for incorrent input type (anything other than Cb::CbApplication)' do
            expect { Cb::ApplicationApi.send(info[:method], Object.new) }.to raise_error Cb::IncomingParamIsWrongTypeException
          end
          ...
          context 'when everything succeeds' do
            ...
            it 'the same application object that it took as input is returned' do
              app = Cb::ApplicationApi.send(info[:method], @app)
              app.should eq @app
          ...
          context 'when missing ResponseApplication field' do
            it 'app redirect url is to a blank string' do
              stub_app_submission_for(info[:uri], Hash.new)
              assert_blank_redirect_url Cb::ApplicationApi.send(info[:method], @app)
            end
          end
          ...
          context 'when ResponseApplication does not have hash content' do
            it 'raises NoMethodError' do
              ...
              expect { Cb::ApplicationApi.send(info[:method], @app) }.to raise_error NoMethodError
            end
          end
        end
