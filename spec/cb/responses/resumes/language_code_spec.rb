...
module Cb
  describe Responses::LanguageCodes do
    ...
    context 'when the api response comes back as expected' do
      let(:codes) { Responses::LanguageCodes.new(response) }
      it { expect(codes.models.first).to be_an_instance_of Models::LanguageCode }
    end
    ...
    context 'when the Api response is missing the collection hash' do
      ...
      it 'raises an exception' do
        ...
        expect{ Responses::LanguageCodes.new(response) }.
