...
describe Cb::Models::JobReport do
  ...
  context 'when the response json comes back correctly' do
    ...
    it 'parses fields correctly' do
      ...
      expect(model.job_information.salary_information.applicant_salaries).to be_a(HireInsider::ApplicantSalary)
      expect(model.job_information.salary_information.degree_salaries).to be_a(HireInsider::DegreeSalary)
      ...
      expect(model.competition_information.experience_ranges[0]).to be_a(HireInsider::Experience)
      expect(model.competition_information.applicant_locations[0]).to be_a(HireInsider::Location)
      expect(model.competition_information.top_five_degrees[0]).to be_a(HireInsider::Degree)
      expect(model.competition_information.top_five_majors[0]).to be_a(HireInsider::Major)
    end
  end
end
